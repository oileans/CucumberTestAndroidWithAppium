...
When('I select a product of the list') do
  @products_screen.select_product_of_the_list('Mac Book')
end

Then('The product should be removed from the list') do
  expect(@products_screen.is_removed_product_from_list?('Mac Book')).to eq(nil)
end
...
Then('It should calculate all products of the list') do
  expect(@products_screen.get_total_of_products).to eq('R$ 15180')
end
